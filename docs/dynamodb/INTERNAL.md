...
> 
> 일관된 해싱과 관련한 내용은 [Consistancy Hashing](../distributedsystem/consistancy-hashing.md)를 참고해주세요.
...
> Ring은 순서가 있기 때문에, 그 다음 노드들에도 데이터를 저장하도록 한다면 결국 N개의 노드에 데이터를 복제해서 저장할 수 있다.
>
> 만약 한 노드가 죽더라도 Ring에서 다음에 위치한 노드에 데이터를 복제하였기 때문에, 그 다음 노드를 알아내서 데이터를 무리없이 가져올 수 있다.

![Figure 2: Partitioning and replication of keys in Dynamo ring.](http://s3.amazonaws.com/wernervogels/public/sosp/sosp-figure2-small.png)
